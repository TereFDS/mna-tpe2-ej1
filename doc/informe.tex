...
\title{M\'etodos Num\'ericos Avanzados\\  Escala Musical }
%
%
...
\author{Mar\'ia de la Puerta Echeverr\'ia (50009),
	...
	Tom\'as Mehdi (51014)
	\\~\IEEEmembership{Grupo 2 - Trabajo Pr\'actico 2 - Instituto Tecnol\'ogico de Buenos Aires (ITBA)}
...
En el presente art\'iculo se describen dos programas: 1) dado un archivo de sonido, indica el tono fundamental del mismo y crea una partitura asociada; y 2) dado una partitura, crea el sonido correspondiente. Ambos est\'an implementados en Matlab y utilizan la escala de temperamento igual.
\end{abstract}

\renewcommand{\IEEEkeywordsname}{Palabras claves}
\begin{IEEEkeywords}
 Temperamento igual, FFT, IFFT, tono, semitono, partitura, escala musical.
\end{IEEEkeywords}}
...
\IEEEPARstart{E}xiste una variedad de escalas musicales que tienen diferentes or\'igenes hist\'oricos. Una de ellas es la escala conocida como de \textit{Temperamento Igual}. \'Esta divide la escala de frecuencias musicales en fragmentos compuestos por doce niveles diferentes$\footnote[1]{Que, en la notaci\'on latina son: do, do$\sharp$, re,  mi$\flat$, mi, fa, fa$\sharp$, sol, la$\flat$, la, si$\flat$, si.}$, donde los s\'imbolos $\flat$ (\textit{bemol} en castellano) y $\sharp$ (\textit{sostenido} en castellano) representan los \textit{semitonos}. Asimismo, en la m\'usica  resulta relevante identificar la nota fundamental de una canci\'on (o \textit{tono})$\footnote[2]{Estas actividades pertenecen al llamado \textit{Ear training}, en ingl\'es.}$ y su partitura, y realizar el proceso inverso (dada una partitura, crear el sonido asociado). En este informe se presenta el dise\~no de dos programas en Matlab que realizan dichas funciones, utilizando la escala de Temperamento igual.\\
En la siguiente secci\'on, Metodolog\'ia Utilizada, se detalla la misma, as\'i como las pruebas realizadas.
...
Cada grupo de doce notas implica duplicar la frecuencia fundamental de la escala (es decir, pasar de una octava a la siguiente) y, en el temperamento igual, la relaci\'on de frecuencias entre notas sucesivas es siempre la misma:
